 ...
 * chess-core.js
 *
 * Pure-data module for Chess Academy.  Contains all chess content that is
 * independent of the React UI:
 *   - `PIECES`     – Unicode symbol map
 *   - `START`      – Standard starting position (8×8 matrix)
 *   - `applyMoves` – Board-state builder (no rule validation)
 *   - `OPENINGS`   – Annotated opening library (4 openings)
 *   - `CONCEPTS`   – Middlegame strategy concepts (12 cards)
 *   - `QUIZZES`    – Knowledge quiz questions (6 questions)
 *
 * All exports are plain values/functions with no React dependencies, making
 * this module fully unit-testable with Node's built-in test runner.
 *
 * @module chess-core
 */
 ...
 *
 * Each move is a plain coordinate object — no chess-rule validation is performed;
